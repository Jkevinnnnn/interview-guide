...
function ScoreCard({
  ...
  const isEvaluating = evaluateStatus === 'PENDING' || evaluateStatus === 'PROCESSING';
  const isFailed = evaluateStatus === 'FAILED';

  if (isEvaluating) {
    return (
      <div className="bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-700 rounded-2xl p-8 text-white">
        <div className="flex flex-col items-center text-center">
          <Loader2 className="w-12 h-12 mb-5 animate-spin text-white/90" />
          <h3 className="text-2xl font-bold mb-3">面试评估生成中</h3>
          <p className="text-white/90 max-w-2xl leading-relaxed">
            AI 正在分批评估你的回答，通常需要 1-3 分钟。你可以停留在本页等待，也可以稍后从面试记录回来查看。
          </p>
        </div>
      </div>
    );
  ...
  if (isFailed) {
    return (
      <div className="bg-gradient-to-br from-rose-600 to-red-700 rounded-2xl p-8 text-white">
        <div className="flex flex-col items-center text-center">
          <AlertCircle className="w-12 h-12 mb-5 text-white/90" />
          <h3 className="text-2xl font-bold mb-3">面试评估失败</h3>
          <p className="text-white/90 max-w-2xl leading-relaxed">
            {evaluateError || '评估任务执行失败，请稍后重试或重新完成一次面试。'}
          </p>
        </div>
      </div>
    );
