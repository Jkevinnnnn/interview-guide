...
function isEvaluating(interview: InterviewWithResume): boolean {
  return interview.evaluateStatus === 'PENDING'
    || interview.evaluateStatus === 'PROCESSING'
    || (isCompletedStatus(interview.status) && !interview.evaluateStatus && interview.overallScore === null);
}
...
export default function InterviewHistoryPage({ onBack: _onBack, onViewInterview }: InterviewHistoryPageProps) {
  ...
  useEffect(() => {
    ...
    if (hasEvaluating && pollingRef.current === null) {
      pollingRef.current = window.setInterval(() => {
        loadAllInterviews(true);
      }, 3000); // 每3秒轮询一次
    } else if (!hasEvaluating) {
                    ...
                    <td className="px-6 py-4">
                      {isEvaluateCompleted(interview) && interview.overallScore !== null ? (
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-blue-500 dark:text-blue-400 text-sm">评估中，约 1-3 分钟</span>
