...
function isEvaluationPending(status?: string | null) {
...
export default function ResumeDetailPage({ resumeId, onBack, onStartInterview }: ResumeDetailPageProps) {
  ...
  const loadSelectedInterviewSilent = useCallback(async (sessionId: string) => {
    try {
  ...
  useEffect(() => {
    const isProcessing = resume && (
      ...
      (resume.interviews || []).some(interview => isEvaluationPending(interview.evaluateStatus))
    );
  ...
  // 面试详情页单独轮询评估状态，避免用户停留在详情页时一直看到旧的“生成中”状态
  useEffect(() => {
    if (
      detailView !== 'interviewDetail' ||
      !selectedInterview ||
      !isEvaluationPending(selectedInterview.evaluateStatus)
    ) {
      return;
    }

    const timer = setInterval(() => {
      loadSelectedInterviewSilent(selectedInterview.sessionId);
    }, 3000);

    return () => clearInterval(timer);
  }, [detailView, selectedInterview, loadSelectedInterviewSilent]);
