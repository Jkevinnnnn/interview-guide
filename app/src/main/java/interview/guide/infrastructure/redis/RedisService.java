...
public class RedisService {
    ...
    public boolean streamConsumeMessages(
        ...
        try {
            ...
            if (isRedissonEmptyStreamRead(e)) {
                log.debug("Stream 阻塞读取超时未返回消息: stream={}, group={}, consumer={}",
    ...
    /**
     * Redisson 4.0 在部分 Redis/协议组合下，XREADGROUP 阻塞超时的空响应可能被解码为 EmptyList，
     * 内部再强转为 Map 时会抛 ClassCastException。这里把该场景视为“本轮无消息”。
     */
    private boolean isRedissonEmptyStreamRead(ClassCastException e) {
