...
public class TextCleaningService {
    ...
    public String cleanText(String text) {
        ...
        String t = repairLikelyMojibake(text);
    ...
    /**
     * 修复部分 PDF/Office 文档中“UTF-8 文本被误按 ISO-8859-1 解码”的典型乱码。
     * 仅在文本明显包含 mojibake 特征，且修复后中文字符占比显著提升时启用。
     */
    private String repairLikelyMojibake(String text) {
        if (!looksLikeMojibake(text)) {
            return text;
        }

        String repaired = new String(text.getBytes(StandardCharsets.ISO_8859_1), StandardCharsets.UTF_8);
        int originalCjk = countCjk(text);
        int repairedCjk = countCjk(repaired);
        if (repairedCjk > Math.max(originalCjk * 2, 10)) {
            return repaired;
        }
        return text;
    }
    ...
    private boolean looksLikeMojibake(String text) {
        int markerCount = 0;
        String markers = "ÃÂåæçèéäöü";
        for (int i = 0; i < text.length(); i++) {
            if (markers.indexOf(text.charAt(i)) >= 0) {
                markerCount++;
            }
        }
        return markerCount > 8 && markerCount > text.length() / 50;
    }
