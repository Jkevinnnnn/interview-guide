 ...
 */
@Slf4j
@RestController  // ①会自动将类中方法返回的这些java对象转换成浏览器可以看懂的JSON格式字符串
                 // ②代码是用来做接口的，而不是展示一个HTML页面
                 // ③不需要在每一个方法上加一个@responsebody注解（responsebody：把方法的返回值直接写入到HTTP相应体中，而不是解析成一个页面去跳转）
@RequiredArgsConstructor  // 自动生成一个包含所有final字段的构造函数，并且配合spring实现依赖注入
                          // ①所有必须的依赖都通过构造函数声明（这个类所有干活的工具ResumeUploadService等全部列出来），确保了Bean在被创建后就处于一个完全初始化、可用的状态（必须要先找到干活的工具，然后才能new），避免因依赖未注入而导致的运行时的错误（如果找不到的话根本就无法new，但是如果采用字段注入就还是可以程序正常启动，但是会报错空指针异常）
                          // ②依赖项通常被声明为final，一旦对象被创建，核心以来就无法修改
                          // ③编写单元测试的时候，不需要启动整个spring容器，通过new的方式手动注入对象，测试更加轻量和便捷
                          // ④两个bean之间存在循环依赖，使用构造器注入会在spring容器启动的时候直接抛出异常
public class ResumeController {
    ...
    // "/api/resumes/upload"这是前端调用的地址 consumes告诉spring这个接口只接受文件上传
    @RateLimit(dimensions = {RateLimit.Dimension.GLOBAL, RateLimit.Dimension.IP}, count = 5)
    // 限流维度，控制全局总次数，限制单个IP的次数
